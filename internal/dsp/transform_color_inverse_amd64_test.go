...
// TestTransformColorInverseSSE2Conformance verifies the SSE2 inverse
...
func TestTransformColorInverseSSE2Conformance(t *testing.T) {
	...
	for iter := 0; iter < 20000; iter++ {
		...
		dstSSE2 := make([]uint32, n+1)
		dstGo[n], dstSSE2[n] = guard, guard

		transformColorInverseGo(g2r, g2b, r2b, src, dstGo[:n], n)
		transformColorInverseSSE2Batch(g2r, g2b, r2b, src, dstSSE2[:n], n)

		for i := range dstGo {
			if dstGo[i] != dstSSE2[i] {
				t.Fatalf("iter %d n=%d mults=(%d,%d,%d): mismatch at dst[%d]: src=%08x go=%08x sse2=%08x",
					iter, n, g2r, g2b, r2b, i, src[i%n], dstGo[i], dstSSE2[i])
			}
		}
	}
}

// TestTransformColorInverseSSE2InPlace mirrors the decoder's in-place call
// (src and dst are the same slice).
func TestTransformColorInverseSSE2InPlace(t *testing.T) {
	...
	for iter := 0; iter < 2000; iter++ {
		...
		transformColorInverseSSE2Batch(g2r, g2b, r2b, buf, buf, n)

		for i := range want {
			if want[i] != buf[i] {
				t.Fatalf("iter %d n=%d: in-place mismatch at [%d]: go=%08x sse2=%08x",
...
func BenchmarkTransformColorInverse(b *testing.B) {
	...
	for _, n := range []int{16, 32, 1024} {
		...
		b.Run("sse2/n"+itoa(n), func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				transformColorInverseSSE2Batch(-52, 37, -110, src, dst, n)
			}
		})
	}
}
